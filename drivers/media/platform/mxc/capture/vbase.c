                               ...
                               struct v4l2_fmtdesc *fmt)
{
  ...
  fmt->pixelformat    = vbase_data.pix.pixelformat;
  fmt->description[0] = vbase_data.pix.pixelformat & 0xFF;
  fmt->description[1] = ( vbase_data.pix.pixelformat >> 8 ) & 0xFF;
  fmt->description[2] = ( vbase_data.pix.pixelformat >> 16 ) & 0xFF;
  fmt->description[3] = ( vbase_data.pix.pixelformat >> 24 ) & 0xFF;
  fmt->description[4] = 0;
                         ...
                         struct v4l2_streamparm *a)
{
  ...
  case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    ...
    vbase_data.pix.pixelformat     = video_input_array[a->parm.capture.capturemode].pixelformat;
                                  ...
                                  struct v4l2_frmsizeenum *fsize)
{
  ...
  if(fsize->index >= num_of_inputs)
    return -EINVAL;

  fsize->pixel_format    = video_input_array[fsize->index].pixelformat;
                       ...
                       const struct i2c_device_id *id)
{
  ...
  dev_err(dev, "%s, I2C device \"%s\", addr 0x%02X\n", __func__, client->name, client->addr);
  ...
  vbase_data.pix.pixelformat = video_input_array[0].pixelformat;
